...
import { proGtfsSourceDisplayName, repoFeedKey } from '../../model/pro-source-helpers'
...
export function GtfsSourcePanel({
  ...
  onRenameSource: (source: ProGtfsSource, displayName: string) => void
  ...
  return (
    <Box sx={{ display: 'grid', gap: 2 }}>
      <Box>
        <Typography variant="h6">GTFSの管理</Typography>
            ...
            <Card key={source.sourceId} variant="outlined" sx={{ p: 1, display: 'flex', justifyContent: 'space-between', gap: 1 }}>
              <Box sx={{ flex: 1, minWidth: 0 }}>
                ...
                <TextField
                  ...
                  placeholder={source.info.name ?? source.info.id}
                  onChange={(event) => onRenameSource(source, event.target.value)}
                  slotProps={{ inputLabel: fieldLabelProps }}
                />
                {source.info.kind === 'repo' && (
                  <TextField
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                ...
                {source.info.kind === 'raw' && (
                  <Tooltip title="別のZIPファイルでGTFSを置換">
      ...
      <Dialog open={Boolean(deleteTarget)} maxWidth="xs" fullWidth onClose={() => setDeleteTarget(null)}>
        ...
        <DialogContent sx={{ display: 'grid', gap: 1.5 }}>
          <Typography>「{deleteTarget ? proGtfsSourceDisplayName(deleteTarget) : 'このGTFS設定'}」を削除します。</Typography>
